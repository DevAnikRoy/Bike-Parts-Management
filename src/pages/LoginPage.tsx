...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      setOtpWait(rateLimitRetryAfter(otpSendKey(email), RATE.otpSend))
    }
    tick()
        ...
        {isLocalDemoMode ? (
          ...
          <>
            ...
            <form onSubmit={(e) => void sendCode(e)}>
              ...
              <button type="submit" className="btn block" disabled={busy || otpBlocked}>
                {busy
                  ? 'পাঠানো হচ্ছে...'
                  : otpBlocked
                    ? `${otpWait} সেকেন্ড পর`
          ...
          <form onSubmit={(e) => void verify(e)}>
            ...
            >
              {otpBlocked ? `${otpWait} সেকেন্ড পর আবার` : 'আবার কোড পাঠান'}
            </button>
            <button
              type="button"
              className="btn ghost block"
